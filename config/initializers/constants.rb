PROVINCES = {"AB" => "AB", "BC" => "BC", "MB" => "MB", "NB" => "NB", "NL" => "NL", "NS" => "NS", 
			"NT" => "NT", "NU" => "NU", "ON" => "ON", "PE" => "PE", "QC" => "QC", "SK" => "SK", "YT" =>"YT"}.freeze

JOB_TYPES = {"Full Time" => 0,"Part Time" => 1,"Contract" => 2,"Casual" => 3,
			 "Summer Positions" => 4, "Field Placement/Work Practicum" => 5,"Volunteer" => 6}.freeze

PAY_RATES = {"Yearly" => "yearly", "Hourly" => "hourly"}.freeze

SURVEYS = { 0 => "General Computer", 1 => "Internet and Networks", 2 => "Programming", 3 => "Word Processing", 
			4 => "Spreadsheet", 5 => "Online Communication and Collaboration", 6 => " Time, Proejct, People", 
			7 => "Money", 8 => "Presentation", 9 => " Multimedia", 10 => "Social Media", 11 => "21st Century Skills" }.freeze

GENDERS = { "Male" => "male", "Female" => "female"}.freeze

LOW_SKILLS = 10.freeze

LOW_PROJECTS= 5.freeze

LOW_REFERENCES = 3.freeze

LOW_JOBS = 1.freeze